...
public class TeleportCommand implements CommandExecutor {
    ...
    private static final Map<UUID, Set<UUID>> tpaBlocks = new HashMap<>();
        ...
        switch (cmd) {
            ...
            case "tpa": {
                ...
                switch (arg) {
                    ...
                    case "block": {
                        if (args.length != 2) {
                            player.sendMessage("§cUso correto: /tpa block <jogador>");
                            return true;
                        }

                        Player toBlock = Bukkit.getPlayerExact(args[1]);
                        if (toBlock == null || !toBlock.isOnline()) {
                            player.sendMessage(mm.getMessage("teleport.player_not_found"));
                            return true;
                        }
                        ...
                        UUID blocker = player.getUniqueId();
                        UUID target = toBlock.getUniqueId();
                        Set<UUID> blocked = tpaBlocks.computeIfAbsent(blocker, k -> new HashSet<>());
                        ...
                        Set<UUID> blockedByTarget = tpaBlocks.getOrDefault(targetPlayer.getUniqueId(), Collections.emptySet());
                        if (blockedByTarget.contains(player.getUniqueId())) {
                            player.sendMessage("§cEsse jogador bloqueou pedidos de teleporte.");
                            return true;
                        }
                        ...
                        Cooldown.INSTANCE.sendActionBar(
